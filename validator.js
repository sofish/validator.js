...
~ function($) {
	...
	var patterns, fields, errorElement, addErrorClass, removeErrorClass, novalidate, validateForm, validateFields, radios, removeFromUnvalidFields, asyncValidate, getVal, aorbValidate, validateReturn, unvalidFields = [], isAsync = true
	...
	asyncValidate = function($item, klass, isErrorOnParent) {
		var data = $item.data(),
			...
			params = data['urlParams'] ? data['urlParams'] : {},
			isOK, pattern, type
		...
		if (val) {
			isOK = (pattern ? (new RegExp(pattern)).test(val) :
				patterns[type](val));
			if (isOK) {
				var ajaxValidateReturn;
				$.ajax({
					type:method,
					url:url,
					data:params,
					...
					success:function(msg) {
						var message = (msg === 'ok' ? 'IM VALIDED' : 'unvalid');
						if (msg !== 'ok') {
							// 将错误信息写入到提示区域
							errorElement($item, isErrorOnParent).find('.validator-unvalid').html(msg)
						}
						ajaxValidateReturn = validateReturn.call(this, $item, klass, isErrorOnParent, message);
					}
				});
				return ajaxValidateReturn;
			} else {
				addErrorClass.call(this, $item, klass, isErrorOnParent)
			}
		} else if (required) {
			addErrorClass.call(this, $item, klass, isErrorOnParent)
		}
	...
	validateReturn = function($item, klass, parent, message) {
		...
		return isError ? (
			...
			addSuccessClass.call(this, $item, 'has-success', parent),
			$item.trigger('after:' + event, $item),
			false
		)
	}
	...
	validate = function($item, klass, parent) {
		...
		// 异步验证则不进行普通验证
		if (async) {
			var xxx = asyncValidate.apply(this, commonArgs);
			console.log(xxx)
			return xxx;
		}
	...
	errorElement = function($item, parent) {
		...
		// if ( p ) {
		//     $item = $item.closest(p)
		// } else if ($item.closest(parent)) {
		//     $item = $item.closest(parent)
		// }
		// return $item
	}

	addSuccessClass = function($item, klass, parent) {
		return errorElement($item, parent).addClass(klass);
	}

	addErrorClass = function($item, klass, parent, emptyClass) {
		emptyClass = emptyClass || ''
		emptyClass = emptyClass.replace('empty', 'has-warning')

		var className = ''
		if (emptyClass.indexOf('warning') > -1) {
			className = emptyClass
		} else {
			className = klass + ' ' + emptyClass
		}
		return errorElement($item, parent).addClass(className);
	}

	removeErrorClass = function($item, klass, parent) {
		removeFromUnvalidFields.call(this, $item);
		return errorElement($item, parent).removeClass(klass + ' empty unvalid has-error has-warning has-success');
	}
